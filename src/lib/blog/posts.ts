...
import {wrapVideosInHtml} from '@/lib/videos/markdownVideos';
import {DEFAULT_AUTHOR_HANDLE} from '@/lib/blog/authors';
...
export function getPaginatedPosts(page: number): {
    ...
    const start = currentPage === 1 ? 0 : FIRST_PAGE_POSTS + (currentPage - 2) * POSTS_PER_PAGE;
...
async function fetchReleaseContributors(tag: string): Promise<string[]> {
    try {
        ...
        const relRes = await fetch('https://api.github.com/repos/TabularisDB/tabularis/releases?per_page=100', {
            headers,
        });
        const allReleases: {
            tag_name: string;
        ...
        const prevTag = idx >= 0 && idx + 1 < releases.length ? releases[idx + 1].tag_name : null;
        if (!prevTag) return [];

        const users = new Set<string>();

        const cmpRes = await fetch(`https://api.github.com/repos/TabularisDB/tabularis/compare/${prevTag}...${tag}`, {
            headers,
        });
        const data: {
        ...
        for (const commit of data.commits ?? []) {
            ...
            if (author?.login && author.type !== 'Bot' && !author.login.endsWith('[bot]')) {
