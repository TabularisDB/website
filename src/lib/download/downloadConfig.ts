...
function nightlyFile(label: string, desc: string, ext: string, matches: (name: string) => boolean): FileOption | null {
...
export const NIGHTLY_PLATFORM_CONFIG: Record<Platform, PlatformConfig> = {
    windows: {
        label: 'Windows',
        options: available([
            nightlyFile('Installer', 'Direct nightly download', '.exe', (name) => name.endsWith('_x64-setup.exe')),
    ...
    linux: {
        ...
        options: available([
            ...
            nightlyFile('Debian / Ubuntu', 'apt-based distros', '.deb', (name) => name.endsWith('_amd64.deb')),
            nightlyFile('Fedora / RHEL', 'rpm-based distros', '.rpm', (name) => name.endsWith('.x86_64.rpm')),
        ]),
    },
};

export function getPlatformConfig(platform: Platform, channel: ReleaseChannel): PlatformConfig {
    return channel === 'nightly' ? NIGHTLY_PLATFORM_CONFIG[platform] : PLATFORM_CONFIG[platform];
}
